...
# Lint as python3
"""Gomuko (noughts and crosses), implemented in Python.
...
class GomukoState(pyspiel.State):
    ...
    def _apply_action(self, action):
        ...
        current_coord = _coord(action)
        self.board[current_coord] = "x" if self._cur_player == 0 else "o"
        if _line_exists(self.board, current_coord):
...
def _line_exists(board, current_coord):
    """
    Checks if a win exists passing through current_coord.
    current_coord: (row, col)
    """
    r, c = current_coord
    player = board[r, c]
    if player == 0:  # Assuming 0 is empty
        return None

    # Define the 4 directions to check: (row_delta, col_delta)
    ...
    for dr, dc in directions:
        ...
        # Check in both directions along the axis (positive and negative)
