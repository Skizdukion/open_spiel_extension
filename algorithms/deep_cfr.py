...
class DeepCFRSolver(policy.Policy):
    ...
    def __init__(
        ...
        # self._embedding_size = len(self._root_node.information_state_tensor(0)) # This for imperfect information
        # Use observation_tensor for perfect information games like Tic-Tac-Toe
    ...
    def save(self, checkpoint_path):
        """Save policy and advantage networks to checkpoint."""
        import os
    ...
    def load(self, checkpoint_path):
        """Load policy and advantage networks from checkpoint."""
        checkpoint = torch.load(checkpoint_path, weights_only=True)
        self._policy_network.load_state_dict(checkpoint["policy_network"])
        for p, state_dict in enumerate(checkpoint["advantage_networks"]):
            self._advantage_networks[p].load_state_dict(state_dict)
    ...
    def solve(self):
        ...
        for _ in trange(self._num_iterations):
            for p in range(self._num_players):
    ...
    def _traverse_game_tree(self, state, player):
        ...
        if state.is_terminal():
            ...
            self._strategy_memories.add(
                StrategyMemory(
                    state.observation_tensor(other_player),
                    self._iteration,
                    strategy,
                )
            )
    ...
    def _sample_action_from_advantage(self, state, player):
        ...
        """
        info_state = state.observation_tensor(player)
        ...
        advantages = [max(0.0, advantage) for advantage in raw_advantages]
    ...
    def action_probabilities(self, state, player_id=None):
        ...
        info_state_vector = np.array(state.observation_tensor(cur_player))
