...
class NFSP(rl_agent.AbstractAgent):
    ...
    def step(self, time_step: rl_environment.TimeStep, is_evaluation: bool = False):
    ...
    def save(self, checkpoint_path: str, save_optimiser: bool = True) -> None:
        """Saves the average policy network and the inner RL agent's q-network.

        Args:
          checkpoint_path: base path for checkpoint files (without extension).
        ...
        # Save RL agent (DQN) network directly
        rl_checkpoint = {
            "q_network": self._rl_agent._q_network.state_dict(),
            "target_q_network": self._rl_agent._target_q_network.state_dict(),
        }
        if save_optimiser:
            rl_checkpoint["optimizer"] = self._rl_agent._optimizer.state_dict()
        torch.save(rl_checkpoint, checkpoint_path + "_rl.pt")
        ...
        if save_optimiser:
            avg_checkpoint["optimiser"] = self._optimizer.state_dict()
        torch.save(avg_checkpoint, checkpoint_path + "_avg.pt")
